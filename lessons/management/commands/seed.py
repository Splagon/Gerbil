...
class Command(BaseCommand):
    ...
    REQUEST_IS_CHILDS_PROBABILITY = 0.8
    ...
    def create_all_random_adult(self):
        user_count = 0
        # creates all the random users
        while user_count < Command.ADULT_USER_COUNT:
            print(f'Seeding adult user {user_count}',  end='\r')
            ...
            try:
                ...
                continue
            user_count += 1
        print('Adult user seeding complete')
    ...
    def create_adult_users(self, username, first_name, last_name, isAdmin, isDirector):
        ...
        # print(Adult.objects.get(username = username))
    ...
    def create_random_children(self):
        ...
        while user_count < Command.CHILDREN_USER_COUNT:
            print(f'Seeding children user {user_count}',  end='\r')
            first_name = self.faker.first_name()
            last_name = self.faker.last_name()
            username = self._username(first_name, last_name)
    ...
    def create_requests(self):
        ...
        while request_count < Command.REQUEST_COUNT:
            ...
            # Allows oyu to tweak how many requests are for children or adults
            if Command.REQUEST_IS_CHILDS_PROBABILITY > random():
                self.create_a_request(self.get_random_child_user())
            else:
    ...
    def get_random_child_user(self):
        index = randint(0, self.allChildUsers.count()-1)
        return self.allChildUsers[index]
